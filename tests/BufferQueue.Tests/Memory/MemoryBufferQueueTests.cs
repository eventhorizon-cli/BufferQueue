...
public class MemoryBufferQueueTests
{
    ...
    public void Equal_Distribution_Load_Balancing_Strategy_For_Consumers()
    {
        ...
        var group2Consumers = queue
            .CreateConsumers(
                new BufferPullConsumerOptions { TopicName = "test", GroupName = "TestGroup2", AutoCommit = false }, 4)
            .ToList();

        var group3Consumers = queue
            .CreateConsumers(
                new BufferPullConsumerOptions { TopicName = "test", GroupName = "TestGroup3", AutoCommit = false }, 5)
            .ToList();

        var group4Consumers = queue
            .CreateConsumers(
                new BufferPullConsumerOptions { TopicName = "test", GroupName = "TestGroup4", AutoCommit = false }, 16)
            .ToList();
